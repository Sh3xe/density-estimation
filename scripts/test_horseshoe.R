...
# plot.sample.2D(data = data, mesh = mesh)  
# Generate the data
n <- 1000
...
estimated_log_dens <- read.csv("cv_csv/cpp#0#LBFGS10#0.010000#horseshoe.csv")
FEMfunction <- FEM(coeff = estimated_log_dens$V0, FEMbasis = FEMbasis)
...
estimated_log_dens <- read.csv("cv_csv/cpp#0#LBFGS10#0.100000#horseshoe.csv")
FEMfunction <- FEM(coeff = estimated_log_dens$V0, FEMbasis = FEMbasis)
evaluation <- eval.FEM(FEM = FEMfunction, locations = mesh.eval$nodes)
estimated_density <- exp(evaluation)
plot.density.2D(X = X, Y = Y, Z = estimated_density, boundary = boundary.domain.2(), colorscale="viridis")

estimated_log_dens <- read.csv("cv_csv/cpp#0#LBFGS10#1.000000#horseshoe.csv")
FEMfunction <- FEM(coeff = estimated_log_dens$V0, FEMbasis = FEMbasis)
evaluation <- eval.FEM(FEM = FEMfunction, locations = mesh.eval$nodes)
estimated_density <- exp(evaluation)
plot.density.2D(X = X, Y = Y, Z = estimated_density, boundary = boundary.domain.2(), colorscale="viridis")

estimated_log_dens <- read.csv("cv_csv/cpp#0#LBFGS10#10.000000#horseshoe.csv")
FEMfunction <- FEM(coeff = estimated_log_dens$V0, FEMbasis = FEMbasis)
evaluation <- eval.FEM(FEM = FEMfunction, locations = mesh.eval$nodes)
estimated_density <- exp(evaluation)
plot.density.2D(X = X, Y = Y, Z = estimated_density, boundary = boundary.domain.2(), colorscale="viridis")
